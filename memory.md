...
__每隔30秒扫描一次，空闲的内存块还给操作系统__ ，当用户使用释放内存变量，或者使用函数clearAllCache释放缓存时，如果512兆的内存全部空闲，则会整体还给操作系统，如果仍有小部分内存在使用，比如512兆的内存块中仍有10兆在使用，则不会归还操作系统。
...
v = NULL
```
除了手动释放变量，当Sessionn关闭时，比如关闭GUI和其他API连接，都会触发对该Session的所有内存进行回收。当通过Web notebook连接时，10分钟内无数据交换，系统会自动回收内存。
...
* __MemAlloc显示值跟操作系统实际显示值有差异__ 
DolphinDB是C++程序，本身需要一些基础的数据结构和内存开销，MemAlloc显示内存不包括这些内存，如果两者显示相差不大，几百兆以内，都属于正常现象。

* __查询时，报告"out of memory"__ 
该异常往往是由于query所需的内存大于系统可提供的内存导致的。可能有以下原因导致1）查询没有加分区过滤条件或者条件太宽，导致单个qeury设计的数据量太大；2）分区不均匀，可能某个分区过大，该分区的数据超过节点配置的最大内存；3）某个用户持有大的变量，导致节点可用的内存很小。

* __查询时，DolphinDB进程退出,没有coredump产生__ 
这种情况往往是，给节点分配的内存超过系统物理内存的限制，操作系统把DolphinDB强制退出。linux上可以通过操作系统的日志查看原因。

* __执行clearAllCache()函数时，MemAlloc没有明显降低__ 
