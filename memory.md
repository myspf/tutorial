...
而分布式表是DolphinDB处理海量数据的基础，也是内存管理的核心。下面通过一个具体的实例来讲解分布式表内存使用。

### 3.1  创建分布式表

为了方便的查看各个节点的内存使用，我们搭建的集群采用2个节点，单副本模式，两层分区，第一层按照DATE，第二层按SYMBOL。10个date，4个symbol，共100个分区，每个分区1000万行，11列（另外9列为LONG类型），所以每个分区共1000 万行 * 80字节/行 = 800M。整个表共4亿行，大小为32GB。
