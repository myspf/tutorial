...
物理服务配置：24core，48线程；内存256G；磁盘1.8T * 12 volumes
...
#### 4.2 流计算配置
生产环境的流数据中心一般部署到独立的物理机器上（集群或者单节点方式部署）。流数据集群配置，每个实际场景需求差异很大。下面以金融领域为例。

物理服务配置：8 core，48线程；内存256G；磁盘1.8T * 12 volumes

+ 发布节点配置
persistenceWorkerNum : 如果只有一个发布节点，建议设置为服务器的逻辑线程数，如果多个发布节点，按照节点个数分配。
maxPersistenceQueueDepth : 根据物理内存，以及该节点上发布表的个数设置。可采用默认设置。
maxPubQueueDepthPerSite : 根据物理内存，以及订阅客户端的多少确定。可采用默认设置。
maxPubConnections : 根据订阅客户端的多少来确定。

+ 订阅节点配置
subExecutors : DolphinDB作为历史数据库，一般配置成支持订阅功能，支持实时数据入库。根据订阅的流表的多少设置。
maxSubQueueDepth : 
maxSubConnections : 
