...
DolphinDB是一个分布式数据库系统，提供了单机模式、集群模式应对不用的硬件环境，同时提供了数据多副本、元数据高可用、流数据高可用、写入强制刷盘、写入缓存等多种配置，来满足不同的高可用需求场景。  

因此在配置系统之前，先进行基本选型，比如采用单机模式还是集群模式，数据副本个数，元数据高可用级别，数据写入策略等。然后再具体配置各个节点的参数，比如内存，磁盘，网络，CPU等配置项。

### 1. 系统模式选型
实际中，每个业务场景硬件配置以及所要解决的问题都差异很大。比如机器配置方面，有些场景只有一台高性能物理机器，而有些场景有多台普通性能PC；在可用性要求方面，有些离线数据分析业务允许偶尔宕机，只要能快速恢复即可，而有些业务对可用性要求极高，7\*24小时在线；在生产环境方面，有些场景配置了系统断电保护模块，而有些工厂环境中，会经常断电而造成宕机；业务方面，有些物联网数据，有大量的列（几千列），而金融领域往往列较少，对写入性能需求不同；
因此，首先根据实际的硬件配置，以及业务对数据可靠性要求，做适合的模式选型。

#### 1.1 单机/集群模式
单机模式适合在单台物理机器上，除了与集群相关的功能，比如多副本、高可用，单机模式支持所有的功能，包括事务、分布式数据库等。单机模式，减少了多节点之间的通信成本，提升了数据读写的性能。  
集群模式适合部署在多台物理机器上，在高可用、多副本、以及扩展性方面都可以灵活配置，有更高的数据安全性保证。集群配置的节点个数，根据物理机器的资源来选择。  

> 选型原则：单物理机一般情况优先使用单机模式，提升性能；个人或小的团队在项目初期验证阶段，可以使用单机模式，快速方便搭建环境；多物理机必须采用集群模式；对数据可靠性、节点扩展性要求高的场景必须采用集群模式；  
#### 1.2 数据多副本
在集群模式下，可以配置数据副本个数，默认是2个副本。副本提供数据高可用功能，当一个机器上的副本损坏或者机器宕机后，另外一个副本可以提供服务，同时对于数据查询请求，多副本也提供负载均衡的作用，系统会选择当前机器负载低的副本执行查询任务，整体提高查询响应。  
多副本会造成磁盘占用空间翻倍，并且写入数据时延稍高。  

> 选型原则：集群模式下，推荐默认2。如果机器数量多，并且对可靠性要求极高，可以配置成3。  
#### 1.3 元数据高可用
元数据高可用，是指controller节点以raft集群的模式配置。controller节点保存分布式数据库的元数据，普通集群模式下，只有一个节点配置成controller，这种情况下，集群存在单点故障，当controller宕机，整个集群不可用。 而元数据高可用模式下，至少要3台物理机器，保证至少两台机器同时工作的情况下，集群仍然可用。  

> 选型原则：集群模式下，根据可靠性要求配置，对于大部分离线分析场景，推荐不配置。对可靠性要求极高场景，比如24小时在线，数据实时分析，推荐配置。  
#### 1.4 流节点高可用
流数据的发布节点上，可用创建多个流数据表，并且接受多个订阅者的订阅。任何启用发布数据功能的节点都可以成为流数据发布节点，因此，发布节点也存在单点故障问题。当发布节点宕机，所有对该发布节点的订阅业务都会订阅失败。  
流节点的高可用模式，也是采用raft 集群，至少需要3台机器（节点），当至少2台机器工作时，订阅发布业务保证可用。根据测试，高可用模式由于数据拷贝到多个节点上，会造成约20%左右的性能下降。  

> 选型原则：流节点上，除了发布数据外，尽量不要执行其他任务，以保证其稳定性。一般情况下，单个节点可以满足生产环境稳定性要求。如业务稳定性要求极高，采用高可模式。
#### 1.5 写入强制刷盘
当数据写入文件时，并没有直接写入到磁盘，而是在操作系统的缓冲页面中。如果操作系统崩溃或掉电，可能会导致数据丢失，默认情况下不启用强制刷盘策略，由操作系统决定刷盘时机。强制刷盘策略保证了数据的安全，但是会降低数据库的写入性能。
在实践中，一种既经济又能提升性能的策略是将数据库log（redo、undo、元数据），写入容量较小但性能较高的SSD盘，将海量数据写入容量大但性能较低的HDD盘。如果实际的应用场景对数据的安全和可靠性要求较高，可以通过将配置参数dataSync的值设置为1来启用强制刷盘策略。

> 选型原则：一般数据中心，云主机都有断电保护系统，可以不启用强制刷盘。经常断电的场景，比如工厂环境，推荐启用强制刷盘策略，保证数据在掉电后也能正确恢复。  
#### 1.6 写入缓存功能
DolphinDB是列式存储，如果每次写入批量较小，那么每次小批量写入大量的文件，会影响磁盘IO性能，也会压缩效果。因此，DolphinDB提供了分区数据写入缓存的功能，写入缓存功能必须与强制刷盘策略一起使用。当缓存中的数据量达到阈值时，系统才会将数据写入磁盘，这样避免了频繁的磁盘操作。  

写入缓存功能，会占用一部分内存来缓存数据，并且一定要与Redo log一起使用，Redo log尽量写到高性能SSD上，不然也会影响性能。
> 选型原则：如果每次写入数据量大，涉及的分区不是很多，而且列不是太多，可以不开启cacheEngine。反之，推荐使用cacheEngine，提升磁盘IO。

系统可用性和性能之间是互斥的，可用性越高，系统复杂度增加，性能下降，反之，性能越高。因此，在实际中，务必根据业务场景对可用性的实际需求，合理选择可用性级别，并不是越高越好。推荐按照单机模式、数据2副本+集群模式、数据2副本+元数据高可用+集群模式的选择顺序来配置集群。写入强制刷盘和写入缓存，根据业务需要来配置。

### 2. 性能配置选项

不论采用单机模式还是集群模式，DolphinDB提供了很多选项来配置节点使用的硬件能力，比如计算能力的最大并发度、可使用内存的最大量、最大网络链接数、流数据发送队列的最大深度等，从硬件能力的角度分类，选项如下：
