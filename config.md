...
因此在搭建系统之前，先进行基本配置选型，比如采用单机模式还是集群模式，数据副本个数，元数据高可用级别，断电数据全部恢复等。然后再具体配置各个节点，比如内存，磁盘，网络，CPU等性能。

### 1. 系统模式选型
实际中，每个业务场景硬件配置以及所要解决的问题都差异很大。比如物理机器配置，有些场景只有一台高性能物理机器，而有些场景有多台普通性能PC；在可用性要求方面，有些离线数据分析业务允许偶尔宕机，只要能快速恢复即可，而有些业务对可用性要求极高，7\*24小时在线；而生产环境方面，有些场景配置了系统断电保护模块，而有些工厂环境中，会经常断电而造成宕机；业务反面，有些物联网数据，有大量的列（几千列），而金融领域往往列较少；
...
集群模式适合有多台物理机器上，同时在高可用、多副本、以及扩展性方面都可以灵活配置，有更高的数据安全性保证。集群配置的节点个数，根据物理机器的资源来选择。
...
#### 1.3 元数据高可用
元数据高可用，是指controller节点以raft集群的模式配置。controller节点保存分布式数据库的元数据，集群模式下，只有一个节点配置成controller，这种情况下，集群存在单点故障，当controller宕机，整个集群不可用。 而元数据高可用模式下，至少要3台物理机器，保证至少两台机器同时工作的情况下，集群仍然可用。  

> 选型原则：集群模式下，根据可靠性要求配置，对于大部分离线分析场景，推荐不配置。对可靠性要求极高场景，比如24小时在线，数据实时分析，推荐配置。  
#### 1.4 流节点高可用
流数据的发布节点上，可用创建多个流数据表，并且接受多个订阅者的订阅。任何启用发布数据功能的节点都可以成为数据发布节点，因此，发布节点也存在单点故障问题。当发布节点宕机，所有对该发布节点的订阅业务都会订阅失败。  
流节点的高可用模式，也是采用raft 集群，至少需要3台机器（节点），当至少2台机器工作时，订阅发布业务保持可用。高可用模式由于数据拷贝到多个节点上，会造成约20%左右的性能下降。  
...
> 选型原则：一般数据中心，云主机都有断电保护系统，可以不启用强制刷盘。经常断电的场景，比如工厂环境，推荐启用强制刷盘策略，保证数据在掉电后也不丢失。  
#### 1.6 写入缓存功能
...
系统可用性和性能之间是互斥的，可用性越高，系统复杂度增加，性能下降，反之，性能越高。因此，在实际中，务必根据业务场景对可用性的实际需求，合理选择可用性级别，并不是越高越好。推荐按照单机模式、数据2副本+集群模式、数据2副本+元数据高可用+集群模式的顺序来配置集群。写入强制刷盘和写入缓存，根据业务需来配置。
