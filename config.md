...
DolphinDB是一个分布式数据库系统，提供了单机模式、集群模式应对不用的硬件环境，同时提供了数据多副本、元数据高可用、流数据高可用、断电数据恢复等多种配置，来满足不同的高可用需求场景。  

因此在搭建系统之前，先进行基本配置选型，比如采用单机模式还是集群模式，数据副本个数，元数据高可用级别，断电数据全部恢复等。然后再具体配置各个节点，比如内存，磁盘，网络，CPU等性能。

### 1. 系统模式选型
实际中，每个业务场景硬件配置以及所要解决的问题都差异很大。比如物理机器配置，有些场景只有一台高性能物理机器，而有些场景有多台普通性能PC；在可用性要求方面，有些离线数据分析业务允许偶尔宕机，只要能快速恢复即可，而有些业务对可用性要求极高，7\*24小时在线；而生产环境方面，有些场景配置了系统断电保护模块，而有些工厂环境中，会经常断电而造成宕机。  
因此，首先根据实际的硬件配置，以及业务对数据可靠性要求，做适合的模式选型。比如采用单机模式还是集群模式，元数据是否高可用，是否支持断电数据恢复等。
...
#### 1.5 断电数据恢复
通过Rodo日志机制，来保证断电后全部数据都能正确恢复。简单来说，写入的数据会先写一份到Redo日志中，并且确保持久化到磁盘上后，才返回给用户写入成功。
Redo日志会异步多写一次数据，并且会flush磁盘，因此能带来数据写入性能的损失。Redo日志一般要写在企业级SSD上，以保证性能，如果是HDD，那么性能降低更加明显。  

> 选型原则：一般数据中心，云主机都有断电保护系统，可以不写Redo日志。一些生产环境较为恶劣场景，比如工厂环境，推荐启用断电数据恢复配置。  

系统可用性和性能之间是互斥的，可用性越高，系统复杂度增加，性能下降，反之，性能越高。因此，在实际中，务必根据业务场景对可用性的实际需求，合理选择可用性级别，并不是越高越好。推荐按照单机模式、数据2副本+集群模式、数据2副本+元数据高可用+集群模式的顺序来配置集群。  
