...
#### 1.3 元数据高可用
元数据高可用，是指controller节点以raft集群的模式配置，至少要3台物理机器。保证至少两台机器同时工作的时候，集群仍然可用。

选型原则：集群模式下，根据可靠性要求配置，对于大部分离线分析场景，推荐不配置。对可靠性要求极高场景，比如24小时在线，数据实时分析，推荐配置。
#### 1.4 流节点高可用
流数据节点的高可用也是采用raft集群模式。可根据对流数据稳定性要求级别配置。

#### 4.5 断电数据恢复
通过rodo log机制，来保证断电后全部数据都能正确恢复。简单来说，写入的数据会先写一份到redo log中，并且确保持久化到磁盘上后，才返回给用户写入成功。
redo log会异步多写一次数据，并且会flush磁盘，因此能带来数据写入性能的损失。

总结，可用性和性能之间是互斥的，可用性越高，系统复杂度增加，性能下降，反之，性能越高。因此，在实际中，务必根据业务场景的可用性要求，合理选择可用性级别，并不是越高越好。推荐按照单机模式、数据2副本集群模式、数据2副本、元数据高可用集群模式的顺序来配置集群。
