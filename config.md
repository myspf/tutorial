...
DolphinDB是一个分布式数据库系统，提供了单机模式、集群模式应对不用的硬件环境，同时提供了数据多副本、元数据高可用、流数据集群等多种配置，来满足不同的高可用需求场景。实际中，每个客户的硬件配置以及生产环境是差异很大的，比如机器配置上，有些客户是单机高性能，有些客户是多台普通PC或者是多台高性能机器；电源保证方面，数据中心提供可靠UPS保证系统不会断电，但物联网场景经常出现因断电而停机；对可靠性要求不同，有些要求必须24小时在线能够提供服务，有些业务允许短时间宕机，在一定时间内恢复就可以。
...
总结，可用性和性能之间是互斥的，可用性越高，系统复杂度增加，性能下降，反之，性能越高。因此，在实际中，务必根据业务场景对可用性的实际需求，合理选择可用性级别，并不是越高越好。推荐按照单机模式、数据2副本+集群模式、数据2副本+元数据高可用+集群模式的顺序来配置集群。  
