...
DolphinDB是一个分布式数据库系统，提供了单机模式、集群模式应对不用的硬件环境，同时根据对数据可靠性的要求，提供了数据多副本、controller集群、流数据集群等多种配置。同时，针对每个节点也提供了一系列配置选项，以充分利用机器硬件资源。合理的参数配置，使系统均衡合理的使用这些资源，最大化发挥机器硬件的性能。

实际中，每个客户的硬件配置以及生产环境是差异很大的，比如机器配置上，有些客户是单机高性能，有些客户是多台普通PC或者是多台高性能机器；电源保证方面，数据中心提供可靠UPS保证系统不会断电，但物联网场景经常出现因断电而停机；对可靠性要求不同，有些要求必须24小时在线能够提供服务，有些业务允许短时间宕机，在一定时间内恢复就可以。

由于各种硬件配置的差异以及对可靠性要求的不同，在对节点具体配置之前，先要根据业务的不同，做一些框架性的选型，比如采用单机模式还是集群模式，配置多少个节点合适，数据副本几个，是否采用元数据高可用，是否需要断电数据全部恢复等。然后在具体配置各个节点，比如内存，磁盘，网络，CPU等性能。

### 1. 选型
#### 1.1 单机模式、集群模式
单机模式适合在单台物理机器上，除了与集群相关的功能，比如多副本、高可用，单机模式支持所有的功能，包括事务、分布式数据库等。单机模式，减少了多节点之间对的通信成本，明显的提升了性能。

集群模式适合有多台物理机器上，同时在高可用、多副本、以及扩展性方面都可以灵活配置，有更高的数据安全性保证。

选型原则：单机一般情况优先使用单机模式，提升性能；多机必须采用集群模式；

#### 1.2 数据多副本
在集群模式下，可以配置数据副本个数，模式是2个副本。副本提供数据高可用功能，当一个机器上的副本损坏或者机器宕机后，另外一个副本可以提供服务，同时对于数据查询请求，多副本也提供负载均衡的作用，系统会选择当前机器负载低的副本执行查询任务。

当然，多副本会造成磁盘占用空间翻倍，并且写入数据时延稍高。

选项原则：集群模式下，推荐默认2。如果机器数量多，并且对可靠性要求极高，可以配置成3；
...
DolphinDB提供了很多选项来配置集群使用的硬件能力，比如计算能力的最大并发度、可使用内存的最大量、最大网络链接数、流数据发送队列的最大深度等，从硬件能力的角度分类，选项如下：
...
DolphinDB 系统记录多种类型的日志，有节点运行情况的日志，数据库元数据日志，redoLog的日志，查询query的性能日志。为了确保系统在异常情况（比如断电等）下的数据安全，有些日志必须同步确保已经持久化到磁盘上。而监控类日志，则不需要严格同步。
